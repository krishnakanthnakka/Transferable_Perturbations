...
class GeneratorModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        #self.logger = logging.getLogger("CDA.trainer")
        #self.eval_logger = logging.getLogger("CDA.inference")
    ...
    def load_eval_model(self):
        ...
        if not self.opt.detcfg.EVAL_MODEL.EVAL_DIFFERENT_MODEL_FROM_TRAIN:
            ...
            #logger.info(" YO model2")

            self.eval_classifier = build_detection_model_eval(
                self.opt.detcfg).cuda()
            #logger.info(" YO model3")
    ...
    def backward_G(self):
        ...
        if 'rl' in self.opt.loss_type:
            loss_rl = -criterion(self.adv_logits -
                                 self.clean_logits, self.pred_label) * self.weight_rl
    ...
    def save_clean_and_adv(self):
        ...
        save_image(clean_images, os.path.join(
            '/cvlabdata1/home/krishna/DA/CDA', 'clean.png'), nrow=4)
       ...
       # adv_images = self.clean255.clone()
        ...
        save_image(self.adv1, os.path.join(
            '/cvlabdata1/home/krishna/DA/CDA', 'adv.png'), nrow=4)

    def evaluate(self, iteration, save_feats):

        self.load_eval_model()
        eval_results = do_evaluation(
            self.opt.detcfg, self.eval_classifier, distributed=False, iteration=iteration, save_feats=save_feats, mean=self.det_mean, det_name=self.opt.detckptname, pooling_type=self.opt.pooling_type, num_images=self.opt.num_images,

            train_config, eval_model, eval_dataset,
    ...
    def evaluate_adv(self, iteration, save_feats):
        ...
        eval_results = do_evaluation_adv(
            self.opt.detcfg, self.eval_classifier, distributed=False, generator=self.netG, eps=self.eps, mean=self.det_mean,
