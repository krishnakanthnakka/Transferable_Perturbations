...
def clamp_channelwise(img, clip_min, clip_max):
    
    initial_x = img.clone()

    img[:,0]=where(img[:,0]>clip_max[0], clip_max[0], img[:,0])
    img[:,1]=where(img[:,1]>clip_max[1], clip_max[1], img[:,1])
    img[:,2]=where(img[:,2]>clip_max[2], clip_max[2], img[:,2])

    img[:,0]=where(img[:,0]<clip_min[0], clip_min[0], img[:,0])
    img[:,1]=where(img[:,1]<clip_min[1], clip_min[1], img[:,1])
    img[:,2]=where(img[:,2]<clip_min[2], clip_min[2], img[:,2])

    return img



def clamp_min(mask, threshold):

    mask = mask.clone()
    mask =where(mask<threshold, 0.0, mask)
...
def normalize_fn(tensor, mean, std):
    ...
    #print(mean.dtype, std.dtype, tensor.dtype)
