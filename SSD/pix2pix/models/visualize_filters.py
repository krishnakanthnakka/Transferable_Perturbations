...
class FilterVisualizer():
    ...
    def visualize(self, layer, filter_index, lr=0.1, opt_steps=20, blur=None):
        ...
        img = (np.random.random((sz, sz, 3)) * 20 + 128.) / 255.0

        #img = (np.random.random((sz, sz, 3)) + 128.)
        # img = np.uint8(np.random.uniform(150, 180, (sz, sz, 3)))  # generate random image

        layer_name = 'feat_{}'.format(layer)

        ic(img.shape, np.max(img), np.min(img))
        ...
        for ii in range(self.upscaling_steps):  # scale the image up upscaling_steps times
            ...
            img_var = normalize_fn(img_var.clone(), PIXEL_MEAN)
            img_var.requires_grad = True

            ic("After norm", img_var.shape, torch.max(img_var), torch.min(img_var))
            ...
            for n in range(opt_steps):  # optimize pixel values for opt_steps times
                ...
                # print(img_var.shape)
                feats = self.model.get_features(img_var, True)
            ...
            img_var = de_normalize_fn(img_var, PIXEL_MEAN)
    ...
    def save(self, layer, filter):
        ...
        if self.save_path is None:
            plt.imsave("layer_" + str(layer) + "_filter_" +
                       str(filter) + ".jpg", np.clip(self.output, 0, 1) / 1.0)
